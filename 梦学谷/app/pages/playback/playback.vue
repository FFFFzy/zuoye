<template>
	<view>
		...
		<view class="content-text">
			<view class="fir-text">
				<view class="left">￥{{ detailList.priceDiscount }}</view>
				<view class="center">
					<s>￥{{ detailList.priceOriginal }}</s>
				</view>
				<view class="last">优惠价</view>
			</view>
			<view class="center-text">{{ detailList.title }}</view>
		...
		<!-- tab导航栏 -->
		<view class="tab-box">
			<view :class="{ 'tab-item': true, active: currentId == item.id }" v-for="item in tabList" :key="item.id" @click="changeTab(item.id)">{{ item.title }}</view>
		</view>
		<view class="tab-content">
			<view class="fir-tab" v-show="currentId == 0"><image :src="detailList.detailUrls" mode=""></image></view>
			<!-- 章节区域 -->
				...
				<view class="big-box">
					<view class="out-box" v-for="(item, index) in articleList" :key="item.id">
						<view class="fir-txt">
							...
							<view class="in-box" v-for="(child, i) in item.sectionList" :key="child.id" @click="toVideo(child)">
			...
			<view class="thr-tab" v-show="currentId == 2">
				<view class="comment-box" v-for="item in commentList" :key="item.id">
					...
					<view class="bot-txt" v-if="item.children != null">讲师回复:{{ item.children.content }}</view>
				</view>
			</view>
			<view class="last-tab" v-show="currentId == 3">
				<view class="last-box" v-for="item in groupList" :key="item.id">
					<view class="fir-group">{{ item.title }}</view>
		...
		<!-- 购买 -->
		<div class="bottom-box"><button class="buy-btn">立即购买</button></div>
	</view>
</template>

<script>
import { reactive, toRefs, ref } from 'vue';
import { getDetailList, getArticle, getComment, getGroup } from '../../utils/api.js';
