<template>
	<view>
		...
		<view class="fir-box">
			<view class="fir-item" v-for="item in articleDetailList.labelList" :key="item.id">{{ item.name }}</view>
			<view class="fir-item">dsifpsh</view>
		</view>
		<view class="text">{{ articleDetailList.title }}</view>
		<view class="pic-box">
			<view class="pic"><image :src="articleDetailList.userImage" mode=""></image></view>
			<span>{{ articleDetailList.nickName }}</span>
			·
			<span>{{ articleDetailList.updateDate }}</span>
			·
...
<script>
import { getArticleDetail, getArticleAsk } from '../../utils/api.js';
...
export default {
	setup() {
		...
		getArticleDetail().then(res => {
			console.log(res);
			data.articleDetailList = res.data.data;
		});
		// 评论数据
		getArticleAsk().then(res => {
			console.log(res);
			data.articleAsk = res.data.data;
		});
		// 返回
		const back = () => {
			// router.push('-1');
